...
For businesses moving money across borders, the appeal is settlement speed. Correspondent banking is slow and expensive at small values, and a token that settles in minutes at any hour addresses a real operational problem rather than an ideological one.

For regulators, the concern is what happens if this scales. If deposits migrate from banks into private tokens, banks lose funding and lend less. The BIS modelled exactly this and found that even at one, two or three trillion dollars in market value, the net effect on output was small and, in its US-calibrated model, slightly negative over the medium term. That is a striking result: the institution's objection is not that stablecoins would be explosive, but that they would impose costs without delivering much growth.

For people in countries with unstable currencies, the significance is more immediate and more uncomfortable. Holding a dollar-referenced token is a rational hedge for an individual and a problem in aggregate, since it erodes monetary sovereignty and reshapes capital flows. The BIS calls this stablecoin dollarisation, and flags it as a particular risk for emerging economies. Readers in Nigeria will recognise the dynamic without needing it explained.
...
Card networks and payment processors have begun offering stablecoin payout and settlement options, which matters because it places the token behind a familiar interface rather than requiring merchants to handle it directly.
...
**The market size figures are softer than they look.** Reported totals for 2026 varied across the low $300 billions depending on the provider and the date. Even the OCC, in its own economic analysis, worked from private-sector forecasts giving an upper bound of $500 billion for 2026 rather than a measured figure. Treat any precise number as an estimate with a methodology attached.

**Concentration.** The market is dominated by two issuers. Compliance costs tend to entrench incumbents rather than diversify a market, and a payment system resting on two private balance sheets carries an obvious single-point-of-failure question.
...
**Compliance is now a continuous obligation.** Under the proposed OCC rules, redemption must be timely, with a proposed limit of two business days. Larger issuers face insured-deposit requirements and audit obligations. Enforcement timelines run into 2027, and these are proposals until finalised, so the detail may shift.
...
What remains unresolved is the question the BIS keeps pressing. A payment instrument can be useful, well-regulated and widely adopted while still not being money in the sense that a monetary system requires, and the difference only becomes visible under stress. The next two years, as US rules finalise and enforcement begins, will test that argument rather than settle it.

For readers tracking how technical systems become financial infrastructure, our analysis of [multimodal AI models](/articles/multimodal-ai-models/) covers a parallel case of a technology outrunning the evidence base, and [the rise of quantum computing](/articles/the-rise-of-quantum-computing/) examines the cryptography that all of this ultimately depends on.
